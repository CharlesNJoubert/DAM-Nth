...
const COMMANDS = {
  ...
  async nav(panel) {
    ...
    const r = await page.evaluate(p => {
      ...
      const btn = btns.find(b => b.getAttribute('title') === p)
      if (!btn) return 'NOT_FOUND: ' + btns.map(b => b.getAttribute('title')).join(', ')
      btn.click(); return 'OK'
    }, panel)
    console.log('nav', panel, '→', r)
...
console.log('Lament Songwriter driver — "help" for commands, "launch" to start')
rl.prompt()
